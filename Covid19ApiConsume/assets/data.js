...
const covidDataBaseURL = 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series'; //APÝ BÝLGÝM
...
const covidDataTypes = {
    confirmed: {
        key: 'confirmed',
        title: 'Vaka',
        dataSourceUrl: `${covidDataBaseURL}/time_series_covid19_confirmed_global.csv`, //CSV 
...
function getTotalRate(covidData, dataTypeKey, regionKeys) {
    if (regionKeys.includes(covidCountries.all.key)) {
        debugger
        const globalTicks = getGlobalTicks(covidData, dataTypeKey);
        if (dataTypeKey == "confirmed") {
            return globalTicks[globalTicks.length - 1];
        }
        const globalTicksConfirmed = getGlobalTicks(covidData, "confirmed");
        const keyValue = globalTicks[globalTicks.length - 1];
        const totalValue = globalTicksConfirmed[globalTicks.length - 1];
        return (keyValue / totalValue).toFixed(2);
    ...
    return regionKeys.reduce((total, regionKey) => {
        ...
        if (!regionTicks) {
            return (total / confirmedRegionTicks).toFixed(2);
        }
        total += regionTicks[regionTicks.length - 1];
        return total;
    }, 0);
