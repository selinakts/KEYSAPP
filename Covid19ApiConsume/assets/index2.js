...
function DataType({ covidData, selectedRegions, dataType, checked, onTypeChange }) {
    ...
    const totalRate = getTotalRate(covidData, dataType.key, selectedRegions);

    document.getElementById(dataType.key).textContent = totalRate;
    const onChange = () => {
    ...
    return (
        e('form', { className: 'form-inline' },
            ...
            e('div', { className: 'form-group form-check mr-3 mb-2' },
                e(Toggle, { checked: groupByCountry, onChange: onGroupByCountries, text: 'Grupla' })
            ),
            e('div', { className: 'form-group form-check mr-3 mb-2' },
                e(Toggle, { text: 'Olcekle', onChange: onUseLogScale, checked: useLogScale })
            ),
...
function RegionsTable({
    ...
    for (let columnIndex = 0; columnIndex < 10; columnIndex += 1) {
        const countryKey = rows[columnIndex].key
        if (document.getElementById(countryKey) != null) {
            document.getElementById(countryKey).style.fill = "#f05154"
        } 
    }
    for (let columnIndex = rows.length - 1; columnIndex > rows.length - 50; columnIndex -= 1) {
        const countryKey = rows[columnIndex].key
        if (document.getElementById(countryKey) != null) {
            document.getElementById(countryKey).style.fill = "#5fd0a5"
        } 
    }
